...
// ======================================
// CHART DATA (Bilancio Plus - EDILCALCESTRUZZO SRL)
// ======================================
// Colori Consistenti
 ...
 const ebitdaData = {
     ...
     datasets: [{
         label: 'EBITDA (€)', data: [654579, 724579],
 ...
 const ebitdaMarginData = {
      labels: ['Attuale (2024)', 'Post Ottimizzazione'],
      datasets: [{
          label: 'EBITDA Margin (%)', data: [8.66, 9.58],
          backgroundColor: [colorAttuale, colorOttimizzato],
          borderColor: [colorAttualeBorder, colorOttimizzatoBorder], borderWidth: 1
      }]
  };
  const pfnEbitdaData = {
       labels: ['Attuale (2024)', 'Post Ottimizzazione'],
       datasets: [{
           label: 'PFN/EBITDA', data: [1.79, 1.55],
           backgroundColor: [colorAttuale, colorOttimizzato],
           borderColor: [colorAttualeBorder, colorOttimizzatoBorder], borderWidth: 1
       }]
   };
   const magazzinoData = {
        labels: ['Attuale (2024)', 'Post Ottimizzazione'],
        datasets: [{
            label: 'Leanus Score', data: [0.52, 1.20],
            backgroundColor: [colorAttuale, colorOttimizzato],
            borderColor: [colorAttualeBorder, colorOttimizzatoBorder], borderWidth: 1
        }]
    };
...
const barChartOptions = {
 ...
 const ebitdaMarginOptions = { ...barChartOptions, scales: { ...barChartOptions.scales, y: { ...barChartOptions.scales.y, suggestedMin: 8, suggestedMax: 10, ticks: { callback: function(v) { return formatPercentage(v); } } } } };
 const pfnEbitdaOptions = { ...barChartOptions, scales: { ...barChartOptions.scales, y: { ...barChartOptions.scales.y, suggestedMin: 1.5, suggestedMax: 2.0, ticks: { callback: function(v) { return formatNumber(v, 2); } } } } };
 const magazzinoOptions = { ...barChartOptions, scales: { ...barChartOptions.scales, y: { ...barChartOptions.scales.y, suggestedMin: 0, suggestedMax: 1.5, ticks: { callback: function(v) { return formatNumber(v, 2); } } } } };

// ======================================
// CHART INITIALIZATION & OTHER SCRIPTS
// ======================================
document.addEventListener('DOMContentLoaded', function() {
     console.log("Initializing scripts for Bilancio Plus EDILCALCESTRUZZO SRL (Embedded)...");
     // --- Initialize Charts ---
     ...
     const irpScoreValue = 64.28; // EDILCALCESTRUZZO SRL IRP
